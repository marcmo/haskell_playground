...
* If only one face of the mineral is sheared, its opposing vertex must be a point (ie. the smooth face must be of odd length), and its two equal-length sides must intersect the shear face at 45 degrees (ie. those sides must cut even-diagonally)
...
foo = print $ "a" ++ pathSeparator:"b"

__ :: [Int] -> Maybe Int
__ xs = trace (unlines $ showMineral xs) Nothing
	
testA = ((==) (Just 10) (__ [15,15,15,15,15]))
 -- 1111      1111
 -- 1111      *111
 -- 1111  ->  **11
 -- 1111      ***1
...
testF = ((==) (Just 9) (__ [18,7,14,14,6,3]))
 -- 10010      10010
 -- 00111      001*0
 -- 01110      01**0
 -- 01110  ->  0***0
...
testG = ((==) Nothing (__ [21,10,21,10]))
 -- 10101      10101
 -- 01010      01010
 -- 10101  ->  10101
 -- 01010      01010
	
testH = ((==) Nothing (__ [0,31,0,31,0]))
 -- 00000      00000
 -- 11111      11111
 -- 00000  ->  00000
 -- 11111      11111
...
showMineral :: [Int] -> [String]
showMineral xs = map showLine xs
  where showLine x = showIntAtBase 2 intToDigit x ""

runTest = all id [testA,testB,testC,testD,testE,testF,testG,testH]
