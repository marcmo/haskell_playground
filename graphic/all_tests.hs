...
----------------------------------------------------------------------
myTests :: [ (String, OutputType -> Renderable ()) ]
myTests =
     [
...
main1 ("--test":tests) = putStrLn "tryout some examples" >> showMyTests tests renderToWindow
...
showMyTests :: [String] -> ((String,OutputType -> Renderable ()) -> IO()) -> IO ()
showMyTests tests ofn = mapM_ ofn (filter (match tests) myTests)
